...
if($id_nivel=="10"){//Promotor
	...
	if(nvl($frm["mode"])=="rechazar_cita_promotor_promotor"){//El promotor, en su agenda, le da eliminar a una cita con otro promotor
		...
		$result=$db->sql_fetchrow($qid);
			...
			$mail->Subject = 'Cancelaci�n de Cita VIA2014';
			$mail->MsgHTML('Mensaje con HTML');
			$template = "Estimado(a) " . $result["destinatario"]. ":\n<br>";
			$template.= "El profesional " . $result["remitente"] . " ha declinado la cita que ten�a programada con usted.\n<br>";
	...
	elseif(nvl($frm["mode"])=="confirmar_cita_promotor_promotor"){//El promotor, en su agenda, le da 'confirmar' a una cita con otro promotor
		...
		$result=$db->sql_fetchrow($qid);
		  ...
		  /*$dest2 = $result["mail_destinatario"];
			...
			$body2.="El promotor " . $result["remitente"] . " ha eliminado la cita que ten�a programada con usted.\n";
			$body2.="La cita qued� entonces programada para la siguiente fecha y hora:\n====\n" . $result["fecha_inicial"] . "\n====\n";
		...
		*/
		/*if($result["mail_destinatario"]!=""){
			...
			$txtMail.="El promotor " . $result["remitente"] . " ha confirmado la cita que ten�a programada con usted.\n";
			$txtMail.="La cita qued� entonces programada para la siguiente fecha y hora:\n====\n" . $result["fecha_inicial"] . "\n====\n";
			$txtMail.="\n<a href=\"http://http://2013.circulart.org/m/index.php?modo=login\">http://2013.circulart.org/</a>\n";
	...
	elseif(nvl($frm["mode"])=="rechazar_cita_promotor"){//El promotor, en su agenda, le da eliminar a una cita
		...
		$result=$db->sql_fetchrow($qid);
		 ...
		 $mail->From = 'via@circulart.org';
				$mail->Subject = 'Cancelaci�n cita VIA2014';
				$mail->MsgHTML('Mensaje con HTML');
				$template = "Estimado(s) " . $result["grupo"] . ":\n<br>";	
				$template.= "El profesional " . $result["promotor"] . " ha declinado la cita que ten�a programada con usted(es).\n<br>";
				$template.= "La cita estaba programada para la siguiente fecha y hora:\n====\n" . $result["fecha_inicial"] . "\n====\n<br>";
				$template.= "\n\n<br>";
				$template.= "Cordial Saludo<br>";
				$template.= "VIA 2014 - Ventana Internacional de las Artes\n<br>";
	...
	elseif($frm["mode"]=="confirmar_cita_promotor"){//El promotor, en su agenda, confirma una cita
		...
		$result=$db->sql_fetchrow($qid);
				...
				$mail->MsgHTML('Mensaje con HTML');
				$template = "Estimado(s) " . $result["grupo"] . ":\n<br>";	
				$template.= "El profesional " . $result["promotor"] . " ha confirmado la cita que ten�a programada con usted(es).\n<br>";
				$template.= "La cita qued� entonces programada para la siguiente fecha y hora:\n====\n" . $result["fecha_inicial"] . "\n====\n<br>";
				$template.= "\n\n<br>";
				$template.= "Cordial Saludo<br>";
				$template.= "VIA 2014 - Ventana Internacional de las Artes\n<br>";
				$template.= "FESTIVAL IBEROAMERICANO DE TEATRO\n<br>";
	...
	if($frm["mode"]=="agenda_artista" || $frm["mode"]=="solicitar_cita_promotor"){
		if($frm["mode"]=="solicitar_cita_promotor"){//Cuando el promotor le solicita una cita al artista
			...
			if($db->sql_numrows($qVerificacion)==0){
				...
				$template.= "La cita solicitada est� programada para la siguiente fecha y hora:\n====\n" . $result["fecha_inicial"] . "\n====\n<br>";
				$template.= "\n\n<br>";
				$template.= "Cordial Saludo<br>";
				$template.= "VIA 2014 - Ventana Internacional de las Artes\n<br>";
				$template.= "FESTIVAL IBEROAMERICANO DE TEATRO\n<br>";
	...
	elseif($frm["mode"]=="agenda_promotor_promotor" || $frm["mode"]=="solicitar_cita_promotor_promotor"){
		if($frm["mode"]=="solicitar_cita_promotor_promotor"){//Cuando el promotor le solicita una cita a otro promotor
			...
			if($db->sql_numrows($qVerificacion)==0){
				...
				$body2.="Por favor confirmar esta cita en la p�gina web.\n";
				$body2.="La cita solicitada est� programada para la siguiente fecha y hora:\n====\n" . $result["fecha_inicial"] . "\n====\n";
				$body2.="\n";
				...
				*/
				...
				/*if($result["prom_email"]!=""){
					...
					$txtMail.="Por favor confirmar esta cita en la p�gina web.\n";
					$txtMail.="La cita solicitada est� programada para la siguiente fecha y hora:\n====\n" . $result["fecha_inicial"] . "\n====\n";
					$txtMail.="\n<a href=\"http://http://2013.circulart.org/m/index.php?modo=login\">http://2013.circulart.org/</a>\n";
...
elseif(in_array($id_nivel,array(4,5,6,7,8,9))){//Grupo
	...
	if(nvl($frm["mode"])=="rechazar_cita_grupo"){
		...
		$result=$db->sql_fetchrow($qid);
				...
				$template.= "El grupo " . $result["grupo"] . " ha declinado la cita que ten�a programada con usted.\n<br>";
				$template.= "La cita estaba programada para la siguiente fecha y hora:\n====\n" . $result["fecha_inicial"] . "\n====\n<br>";
				$template.= "\n\n<br>";
				$template.= "Cordial Saludo<br>";
				$template.= "VIA 2014 - Ventana Internacional de las Artes\n<br>";
	...
	elseif($frm["mode"]=="confirmar_cita_grupo"){//Cuando el grupo desde su agenda, confirma una cita que le ha solicitado el promotor
		...
		$result=$db->sql_fetchrow($qid);
		        ...
		        $mail->From = 'via@festivaldeteatro.com.co';
				$mail->Subject = 'Confirmaci�n cita VIA2014';
				$mail->MsgHTML('Mensaje con HTML');
				$template = "Estimado(a) " . $result["promotor"] . ":\n<br>";	
				$template.= "El grupo " . $result["grupo"] . " ha confirmado la cita que ten�a programada con usted.\n<br>";
				$template.= "La cita qued� entonces programada para la siguiente fecha y hora:\n====\n" . $result["fecha_inicial"] . "\n====\n<br>";
				$template.= "\n\n<br>";
				$template.= "Cordial Saludo<br>";
				$template.= "VIA 2014 - Ventana Internacional de las Artes\n<br>";
    	...
    	if($frm["mode"]=="solicitar_cita_grupo"){//Cuando el artista le solicita una cita al promotor
	     ...
	     if($_SESSION[$CFG->sesion]["user"]["c"]<18){//bloqueo de citas artistas
			...
			if($db->sql_numrows($qVerificacion)==0){
			 ...
			 echo "<script>";
			 echo "alert ('Lo sentimos usted ha agotado el n�mero de citas')";
			 echo "</script>";
			 }
...
function mostrar_agenda_promotor_promotor($frm){
	...
	while($sesion=$db->sql_fetchrow($qSesiones)){
		...
		echo "<tr style='font-size:14px; color:#cc0; text-align: center;'><th width=\"100\">Hora</th><th width=\"210\">Profesional / Artista / Grupo</th><th width=\"90\">Estado</th></tr>\n";
				...
				if($cita=$db->sql_fetchrow($qCita)){
					...
					if($db->sql_numrows($qBloqueo)!=0){//Horario bloqueado
						...
						if($cita=$db->sql_fetchrow($qCita)){
							...
							echo "<tr class='linea'><th scope=\"row\" style='text-align:left'>" . strftime("%H:%M ",$desde) . date("a",$desde) . "</th>\n";
							echo "<td></td><td><a style=\"padding:0px; color:#ccc; text-decoration:underline; text-align:left; border:none; background:none; cursor:pointer;\" href=\"" . simple_me($ME) . "?mercado=".$CFG->mercado."&modo=agenda&mode=solicitar_cita_promotor_promotor&fecha=" . urlencode(date("Y-m-d H:i:s",$desde)) . "&id_sesion=$sesion[id_sesion]&id_promotor=$frm[id_promotor]&id_promotor2=$frm[id_promotor2]&id_mercado=".$CFG->mercado."&act=1\">Solicitar cita</a>";
						}
					}
				}
			}
...
$promotores=$db->sql_row("SELECT * FROM promotores WHERE id='$_GET[id_promotor2]'");
	...
	echo "<div class='titulo'><strong>Visualizaci�n de espacios libres de ".$promotores["nombre"]." ".$promotores["apellido"]."</strong></div>";
	...
	while($sesion=$db->sql_fetchrow($qSesiones)){
		...
		echo "<strong>Fecha:</strong> " . strftime("%A %d de %B",strtotime($sesion["fecha_inicial"])) . "</p>\n";
		echo "<table  border=\"0\" cellpadding=\"5\" cellspacing=\"5\">\n";
		echo "<tr style='font-size:14px; color:#cc0; text-align: center;'><th width=\"100\" style='text-align:left'>Hora</th><th width=\"210\">Profesional / Artista / Grupo</th><th width=\"90\">Estado</th></tr>\n";
		$desde=strtotime($sesion["fecha_inicial"]);
		$hasta=strtotime($sesion["fecha_final"]);
...
function mostrar_agenda_promotor_artista($frm){
	...
	echo "<div class='titulo'><strong>Agenda del artista</strong></div>";
	...
	while($sesion=$db->sql_fetchrow($qSesiones)){
		...
		echo "<strong>Fecha:</strong> " .strftime("%A %d de %B", strtotime($sesion["fecha_inicial"])). "</p>\n";
		echo "<table border=\"0\" cellpadding=\"5\" cellspacing=\"5\">\n";
		echo "<tr style='font-size:14px; color:#cc0; text-align: center;'><th width=\"100\">Hora</th><th width=\"210\">Profesional</th><th width=\"140\">Estado</th></tr>\n";
		$desde=strtotime($sesion["fecha_inicial"]);
		$hasta=strtotime($sesion["fecha_final"]);
		...
		while($desde<$hasta){
			...
			if($cita=$db->sql_fetchrow($qCita)){
				...
				");
				if($db->sql_numrows($qBloqueo)!=0){//Horario bloqueado
				    echo "<tr class='linea' style='border:none; background:#1f1f1f; text-align: left; color:#ccc; font-size: 14px;'><th scope=\"row\" style='text-align:left'>" . strftime("%H:%M ",$desde) . date("a",$desde) . "</th>\n";
					echo "<td style='border:none; background:#1f1f1f; text-align: left; color:#ccc; font-size: 14px;'>&nbsp;&nbsp;<strong>Bloqueado</strong></th>";
					echo "<td style='border:none; background:#1f1f1f; text-align: left; color:#ccc; font-size: 14px;'>&nbsp;&nbsp;<strong>Bloqueado</strong></th>\n";
				}
				else{
					echo "<tr class='linea'><th scope=\"row\" style='text-align:left'>" . strftime("%H:%M ",$desde) . date("a",$desde) . "</th>\n";
					echo "<td></td><td><a style=\"color:#ccc; text-decoration:underline; text-align:left; border:none; background:none; cursor:pointer;\"  href=\"" . simple_me($ME) . "?mercado=".$CFG->mercado."&modo=agenda&mode=solicitar_cita_promotor&fecha=" . urlencode(date("Y-m-d H:i:s",$desde)) . "&id_sesion=$sesion[id_sesion]&tipo=$frm[tipo]&id_artista=$frm[id_artista]&act=0&id_mercado=$_GET[mercado]&id_promotor=$_GET[id_promotor]&id_artista=$_GET[id_artista]\">Solicitar cita</a>";
				}
			}
			echo "</tr>\n";
			$desde+=60*20;
	...
	echo "<div class='titulo'><strong>Visualizaci�n de la agenda de ".$promotores["nombre"]." ".$promotores["apellido"]."</strong></div>";
	...
	while($sesion=$db->sql_fetchrow($qSesiones)){
		...
		echo "<strong>Fecha:</strong> " .strftime("%A %d de %B", strtotime($sesion["fecha_inicial"])). "</p>\n";
		echo "<table border=\"0\" cellpadding=\"5\" cellspacing=\"5\">\n";
		echo "<tr style='font-size:14px; color:#cc0; text-align: center;'><th width=\"100\">Hora</th><th width=\"210\">Artista</th><th width=\"90\">Estado</th></tr>\n";
		$desde=strtotime($sesion["fecha_inicial"]);
		$hasta=strtotime($sesion["fecha_final"]);
		...
		while($desde<$hasta){
			...
			if($cita=$db->sql_fetchrow($qCita)){
				...
				if($cita=$db->sql_fetchrow($qCita)){
					...
					if($db->sql_numrows($qBloqueo)!=0){//Horario bloqueado
					    echo "<tr class='linea' style='border:none; background:#1f1f1f; text-align: left; color:#ccc; font-size: 14px;'><th scope=\"row\" style='text-align:left'>" . strftime("%H:%M ",$desde) . date("a",$desde);
						echo "<td style='border:none; background:#1f1f1f; text-align: left; color:#ccc; font-size: 14px;'>&nbsp;&nbsp;<strong style='border:none;'>Bloqueado</strong>";
						echo "<td style='border:none; background:#1f1f1f; text-align: left; color:#ccc; font-size: 14px;'>&nbsp;&nbsp;<strong style='border:none;'>Bloqueado</strong>";
						echo "</th>\n";
					}
					else{
...
function mostrar_agenda_promotor_promotor_recordatorio($frm){
	...
	$qSesiones=$db->sql_query("
		...
		ORDER BY s.fecha_inicial
	");
	echo "<div style='padding:5px; background-color:#666666; color:#E6D7B0; font-size:14px' align='center'><strong>Visualizaci�n de horarios libres en mi agenda</strong></div>";
	$cambio=0;
	$cambio2=0;
	while($sesion=$db->sql_fetchrow($qSesiones)){
		...
		while($desde<$hasta){
			...
			if($cita=$db->sql_fetchrow($qCita)){
				...
				if($cita=$db->sql_fetchrow($qCita)){
					echo "<tr><th scope=\"row\" class='linea'>" . strftime("%H:%M ",$desde) . date("a",$desde);
					echo "</th>\n";
					echo "<td class='linea'>$cita[promotor] (Profesional)</td>";
					echo "<td class='linea'>$cita[mesa]</td>";
					if($cita["aceptada_promotor"]==1 && $cita["aceptada_promotor2"]==1){ $estado="<span class='confirmado'>Aceptada</span>";}
					elseif($cita["aceptada_promotor"]==1 && $cita["aceptada_promotor2"]==0){ $estado="<span class='porconfirmar'>Por confirmar</span>";}
					elseif($cita["aceptada_promotor"]==0 && $cita["aceptada_promotor2"]==1){ 
					if($cita["id_promotor2"]==$frm["id_promotor"]){
					    $estado="<span class='porconfirmar'>Por confirmar</span>";
					 }else{
						 $estado="<a href=\"" . simple_me($ME) . "?act=2&mercado=".$CFG->mercado."&modo=agenda&mode=confirmar_cita_promotor_promotor&id_cita=$cita[id]\" style='color:#ccc; text-decoration:underline'>Confirmar</a>";
						 }
					}else{ $estado="Eliminada";};
					echo "<td class='linea' style='text-align:center'>$estado</td>";
...
function detalle_mercado($frm){
	...
	if($mercado=$db->sql_fetchrow($qMercado)){
		...
		elseif($frm["mode"]=="agenda_promotor"){
			...
			$string.= "<div class='profesionales' ><p style='height: 140px; overflow:hidden;'><strong>Organizaci�n: </strong>" .$nombreEmpresa."<br><strong>Cargo: </strong>".$promotor["cargo"]."<br>".$resenaEmpresa . "</p>\n";
			$string.= "<a class='vermas' style='margin-left:0;' href='http://via.festivaldeteatro.com.co/perfilp.php?n=".$promotor["id"]."' target='_blank'>Ver m�s</a>";
...
function listar_grupos($frm){
	...
	echo "<br><br><div id='contenido_lista' style='display: none;'><div>Artista:</div>";
	$j=1;
	echo "\n<table>";
	echo "<tr><td>";
	while($gm=$db->sql_fetchrow($qGrupos)){
		if($gm["id_grupo_musica"]!=""){$tipo="musica";$tipoTxt="M�sica";$id=$gm["id_grupo_musica"];}
...
function listar_promotores($frm){
	...
	while($promotor=$db->sql_fetchrow($qPromotores)){ 
		...
		if($promotor["id"]!=4127){ //oculta el profesional de prueba
...
function mostrar_agenda_grupo($frm){
	...
	while($sesion=$db->sql_fetchrow($qSesiones)){
		...
		echo "<tr style='font-size:14px; color:#cc0'><th width=\"100\">Hora</th><th width=\"208\">Profesional</th><th width=\"50\">Mesa</th><th width=\"90\">Estado</th><th width=\"50\">Declinar</th></tr>\n";
		$desde=strtotime($sesion["fecha_inicial"]);
		$hasta=strtotime($sesion["fecha_final"]);
...
//------
	...
	while($sesion=$db->sql_fetchrow($qSesiones)){
		...
		echo "<strong>Fecha:</strong> " . strftime("%B %d de %Y",strtotime($sesion["fecha_inicial"])) . "</p>\n";
		echo "<table  border=\"0\" cellpadding=\"5\" cellspacing=\"5\">\n";
		echo "<tr style='font-size:14px; color:#cc0; text-align: center;'><th width=\"100\">Hora</th><th width=\"208\">Grupo</th><th width=\"90\">Estado</th></tr>\n";
		$desde=strtotime($sesion["fecha_inicial"]);
		$hasta=strtotime($sesion["fecha_final"]);
	...
	echo '<div class="titulo" align="center" ><strong>Visualizaci�n de horarios libres en mi agenda</strong></div>';
	$qSesiones=$db->sql_query("
		SELECT s.id as id_sesion, s.id_rueda, s.lugar, r.nombre, r.duracion_cita, s.fecha_inicial, fecha_final
		FROM sesiones s LEFT JOIN ruedas r ON s.id_rueda=r.id
		WHERE r.id_mercado='$frm[id_mercado]'
	...
	while($sesion=$db->sql_fetchrow($qSesiones)){
		...
		echo "<strong>Fecha:</strong> " . strftime("%B %d de %Y",strtotime($sesion["fecha_inicial"])) . "</p>\n";
		echo "<table  border=\"0\" cellpadding=\"5\" cellspacing=\"5\" >\n";
		echo "<tr style='font-size:14px; color:#cc0; text-align: center;'><th width=\"100\">Hora</th><th width=\"208\">Promotor</th><th width=\"90\">Estado</th></tr>\n";
		$desde=strtotime($sesion["fecha_inicial"]);
		$hasta=strtotime($sesion["fecha_final"]);
