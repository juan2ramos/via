...
<title>Altavoz <?if(isset($titulo)) echo ":: " . $titulo?></title>
<link href="../estilos.css" rel="stylesheet" type="text/css" />
<style type="text/css">
<!--
body {
	background-image: url(../imagenes/altavoz_fondo.gif);
}
...
<div id="cabezote">
  <img src="../imagenes/cabezote_altavoz.jpg" width="1000" height="100" border="0" usemap="#Menu" /> 
  ...
  <a href="grupos.php?artistas=musica"><?=translate($arrTitulos,"grupos")?></a> | 
  <a href="promotores.php">Promotores</a> | 
  <a href="programacion.php">Programaci&oacute;n</a> | 
    ...
    <?
				if(!isset($_SESSION[$CFG->sesion]["user"]["id_nivel"])) echo "<input type=\"button\" onClick=\"window.location.href='login.php'\" value=\"".translate($arrTitulos,"entrar")."\">\n";
					...
					if($_SESSION[$CFG->sesion]["user"]["id_nivel"]==10){//Promotor
						...
						echo "<option value='agenda.php?id_mercado=" . $CFG->id_mercado_3 . "'$selected>".translate($arrTitulos,"miagenda")."</option>";
//						echo "<a href=\"promotores.php?mode=perfil&id=$prom[id]\">$prom[nombre] $prom[apellido]</a> | ";
					...
					elseif(in_array($_SESSION[$CFG->sesion]["user"]["id_nivel"],array(4,5,6,7,8,9))){//Artista
						//preguntar($_SESSION[$CFG->sesion]["user"]);
						...
						$hoy=date("Y-m-d");
						$qMercados=$db->sql_query("SELECT * FROM mercados WHERE fecha_final>='$hoy' AND id IN (SELECT id_mercado FROM mercado_artistas WHERE id_grupo_$art[grupo_tipo]='$art[grupo_id]')");
