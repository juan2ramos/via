    ...
    <body class="contact">
        ...
        <div class="via">
	        <section class="form">
			   ...
			   $db = new MySQL();
			   $qGrupos=$db->consulta("SELECT gt.* FROM grupos_danza gt where id='".$_GET["n"]."'");
			  
			   if($db->num_rows($qGrupos)>0){
				while($gt=mysql_fetch_array($qGrupos)){
			     $caratula=$db->consulta("SELECT * FROM archivos_grupos_danza WHERE id_grupos_danza='".$gt["id"]."' AND orden='0'");
				...
				<div class="perfil">
                  ...
                  <div id="trayectoria" class="text"><strong>trayectoria: </strong><a href="http://circulart.org/admin/fileFS.php?table=grupos_danza&field=trayectoria&id=<?=$gt["id"]?>">ver</a><hr></div>
				 ...
				 <?php 
					$cont=0;
					$obrasD = $db->consulta("SELECT * FROM obras_danza WHERE id_grupos_danza ='".$gt["id"]."'ORDER BY anio DESC");
