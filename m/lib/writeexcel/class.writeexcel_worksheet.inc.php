...
//		preguntar($_);die;
    ...
    function _process_bitmap($bitmap) {
        ...
        $bmp=fopen($bitmap, "rb");
        if (!$bmp) {
					die("Could not open file '$bitmap'.");
        ...
        if (substr($data, 0, 2) != "BM") {
      		die("$bitmap doesn't appear to to be a ".  "valid bitmap image.");  
					trigger_error("$bitmap doesn't appear to to be a ".  "valid bitmap image.", E_USER_ERROR);
        }
        ...
        if ($width > 0xFFFF) {
					die("$bitmap: largest image width supported is 64k.");
            trigger_error("$bitmap: largest image width supported is 64k.", E_USER_ERROR);
        }

        if ($height > 0xFFFF) {
					die("$bitmap: largest image height supported is 64k.");
